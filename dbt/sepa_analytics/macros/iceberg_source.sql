{% macro iceberg_source(table_name) %}
  {#
    ...
    - DuckDB (dev): reads Parquet data files directly from RustFS with Hive partitioning.
  ...
  {% if target.type == 'duckdb' %}
    read_parquet('s3://{{ env_var("RUSTFS_BUCKET", env_var("MINIO_BUCKET", "sepa-lakehouse")) }}/silver/iceberg/sepa/{{ table_name }}/data/**/*.parquet', hive_partitioning = true, union_by_name = true)
  {% else %}
