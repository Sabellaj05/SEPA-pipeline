...
def get_silver_schema_dict(table: str) -> Dict[str, type[pl.DataType]]:
    ...
    if table not in schemas:
        raise ValueError(
            f"Unknown Silver table: '{table}'. Expected one of {list(schemas.keys())}"
        )
...
def to_silver_precios(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if "ean" in df.columns:
        cast_exprs.append(
            (pl.col("ean").cast(pl.Utf8).str.strip_chars() == "1").alias("ean")
        )
    for col in (
    ...
    return _project_to_silver(
        df, SILVER_PRECIOS_SCHEMA, exclude=("fecha_vigencia", "scraped_at")
    )
...
def to_silver_sucursales(df: pl.DataFrame) -> pl.DataFrame:
    ...
    """
    df = df.rename(
        {k: v for k, v in SUCURSALES_RAW_TO_SILVER.items() if k in df.columns}
    )
    ...
    if "provincia" in df.columns:
        cast_exprs.append(
            pl.col("provincia")
            .str.replace_all("^Buenos Aires$", "AR-B")
            .alias("provincia")
        )
    if cast_exprs:
        df = df.with_columns(cast_exprs)
    ...
    return _project_to_silver(
        df, SILVER_DIM_SUCURSALES_SCHEMA, exclude=("fecha_vigencia",)
    )
...
def to_silver_comercios(df: pl.DataFrame) -> pl.DataFrame:
    ...
    """
    df = df.rename(
        {k: v for k, v in COMERCIOS_RAW_TO_SILVER.items() if k in df.columns}
    )
    return _project_to_silver(
        df, SILVER_DIM_COMERCIOS_SCHEMA, exclude=("fecha_vigencia",)
    )
...
def to_silver_productos(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if "cantidad_presentacion" in df.columns:
        cast_exprs.append(
            pl.col("cantidad_presentacion").cast(pl.Float64, strict=False)
        )
    if "marca" in df.columns:
        cast_exprs.append(pl.col("marca").fill_null("S/D").alias("marca"))
    if cast_exprs:
    ...
    return _project_to_silver(
        df, SILVER_DIM_PRODUCTOS_SCHEMA, exclude=("fecha_vigencia",)
    )
